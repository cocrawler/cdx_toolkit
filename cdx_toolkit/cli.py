...
def main(args=None):
    ...
    warc_by_cdx.add_argument('--prefix', default='TEST', help='prefix for the warc filename')
    warc_by_cdx.add_argument('--subprefix', type=str, default=None, help='subprefix for the warc filename, default None')
    warc_by_cdx.add_argument('--size', type=int, default=1000000000, help='target for the warc filesize in bytes')
    warc_by_cdx.add_argument('--creator', action='store', help='creator of the warc: person, organization, service')
    warc_by_cdx.add_argument('--operator', action='store', help='a person, if the creator is an organization')
    warc_by_cdx.add_argument('--warc-download-prefix', action='store', help='prefix for downloading content, automatically set for CC')
    warc_by_cdx.add_argument('index_path')
    warc_by_cdx.set_defaults(func=warcer_by_cdx)
...
def warcer_by_cdx(cmd, cmdline):
    ...
    def get_caputure_objects_from_index_file(index_path: str, warc_download_prefix=None) -> Iterable[cdx_toolkit.CaptureObject]:
        ...
        with smart_open.open(index_path) as f:
    ...
    for obj in get_caputure_objects_from_index_file(index_path=cmd.index_path, warc_download_prefix=cmd.warc_download_prefix):
        url = obj['url']
